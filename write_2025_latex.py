#!/usr/bin/env python3
"""Write the 31 ISL-2025 problems as clean LaTeX from the official booklet."""
from pathlib import Path


P = {
    ("A1", "Czech Republic"): r"""Quadratic solitaire is a single-player game. To start the game, the player chooses two distinct nonzero integers $a$ and $b$ and writes the equation
\[
x^2+ax+b=0
\]
...
Determine all initial choices of $a$ and $b$ such that quadratic solitaire can be played forever.""",
    ...
    ("A2", "China"): r"""The sunshine cost of a sequence $a_1,a_2,\ldots,a_{100}$ of integers is the largest possible value of
\[
...
Determine the smallest possible sunshine cost over all sequences $a_1,a_2,\ldots,a_{100}$ of pairwise distinct integers.""",
    ...
    ("A3", "Italy"): r"""Coral and Joey are playing the inekoalaty game, a two-player game whose rules depend on a positive real number $\lambda$ which is known to both players. On the $n$th turn of the game, the following happens:
\begin{itemize}
...
Determine all values of $\lambda$ for which Coral has a winning strategy and all those for which Joey has a winning strategy.""",
    ...
    ("A4", "Mongolia"): r"""Let $\mathbb{Z}_{\geq 0}$ be the set of all nonnegative integers. Let $f:\mathbb{Z}_{\geq 0}\to\mathbb{Z}_{\geq 0}$ be an unbounded function such that, if $m$ and $n$ are nonnegative integers satisfying
\[
...
We say that $f$ is unbounded if for each nonnegative integer $N$, there exists some nonnegative integer $n$ such that $f(n)\geq N$.""",
    ...
    ("A5", "Thailand"): r"""Determine all integers $n\geq 2$ such that the polynomial
\[
...
can be written as $P(x,y,z)=Q(x,y,z)R(x,y,z)$, where $Q(x,y,z)$ and $R(x,y,z)$ are nonconstant polynomials with integer coefficients.""",
    ...
    ("A6", "Singapore"): r"""Let $S$ be a set of positive integers, possibly infinite, such that no positive integer greater than $1$ divides all elements of $S$. Determine all non-periodic infinite sequences $a_1,a_2,a_3,\ldots$ of positive integers such that, for all positive integers $n$,
\begin{itemize}
...
We say that an infinite sequence $a_1,a_2,a_3,\ldots$ is periodic if there exists a positive integer $t$ such that $a_n=a_{n+t}$ for all positive integers $n$.""",
    ...
    ("A7", "Thailand"): r"""For each integer $k\geq 3$, prove that there exists a unique tuple $(x_1,x_2,\ldots,x_k)$ of positive real numbers such that $x_1\geq x_2\geq\cdots\geq x_k$ and
\[
...
Here $\lfloor z\rfloor$ denotes the greatest integer less than or equal to $z$. For example, $\lfloor-\pi\rfloor=-4$ and $\lfloor2\rfloor=\lfloor2.9\rfloor=2$.""",
    ...
    ("A8", "France"): r"""Tim and Tam play a game. To start the game, Tim writes some nonzero real numbers, not necessarily distinct, on a blackboard. In each round, the following happens:
\begin{itemize}
...
Determine whether Tim can ensure that the game is never stopped, no matter what Tam does.""",
    ...
    ("C1", "U.S.A."): r"""Let $n\geq 3$ be an integer, and let $S_n$ be the set of points $(x,y)$ in the plane such that $x$ and $y$ are nonnegative integers and $x+y<n$. A line in the plane is called interesting if it is not parallel to the $x$-axis, the $y$-axis, or the line $x+y=0$.
...
\end{itemize}""",
    ...
    ("C2", "Colombia"): r"""There is a row of $n$ paddocks, labelled from left to right with the integers $1$ to $n$, where $n\geq 3$. Skippy the Kangaroo grazes in the paddocks according to the following rule:
...
Skippy continues grazing in this way indefinitely. A paddock is overgrazed if Skippy eventually grazes in it, regardless of the paddock that she starts in. Determine all $n\geq 3$ for which there is exactly one overgrazed paddock.""",
    ...
    ("C3", "Islamic Republic of Iran"): r"""There are $2025$ white balls and $2025$ black balls arranged in a row. A balanced segment is a contiguous nonempty segment of balls that contains the same number of white balls as black balls. A balanced removal is the operation of selecting a balanced segment $S$, removing all the balls in $S$, and shifting left the balls that were to the right of $S$ so that the remaining balls form a contiguous row.
...
Determine the smallest positive integer $N$ satisfying the following property: for every configuration of balls and for every integer $k$ satisfying $0\leq k\leq 2025$, there exists a sequence of at most $N$ balanced removals after which there are exactly $2k$ remaining balls.""",
    ...
    ("C4", "China"): r"""Bluey is placing lollies on a $1000\times1000$ grid. Initially, every cell of the grid is empty. At each step, Bluey chooses a row or column in which the total number of lollies is a multiple of $3$, chooses an empty cell in this row or column, and places a lolly in it. If there is no such row or column, Bluey stops.

Determine the maximum number of lollies that Bluey can place.""",

    ("C5", "Germany"): r"""Let $n$ be a positive integer, and let $(a_1,a_2,\ldots,a_n)$ be any $n$-tuple of distinct integers. An inversion is a pair $(i,j)$ of integers satisfying $1\leq i<j\leq n$ and $a_i>a_j$. An inversion $(i,j)$ is called odd if $j-i$ is odd. Let $A$ be the number of all inversions, and let $B$ be the number of odd inversions.
...
\end{itemize}""",
    ...
    ("C7", "China"): r"""Let $P$ be a regular $100$-gon. An Aussie triangulation is formed by dividing $P$ into a set $\mathcal{T}$ of non-overlapping triangles such that:
\begin{itemize}
...
Determine the minimum number of quokkalaterals among all Aussie triangulations.""",
    ...
    ("C8", "Singapore"): r"""Consider a $2025\times2025$ grid of unit squares. We place on the grid some number of rectangular tiles, possibly of different sizes, such that each side of every tile lies on a grid line and every unit square is covered by at most one tile.
...
Prove that line $RO$ bisects angle $\angle DRB$.""",
    ...
    ("G6", "India"): r"""Let $ABC$ be a triangle with $\angle B>\angle A>\angle C$. Let $\Omega$ be the circumcircle of triangle $ABC$. The lines tangent to $\Omega$ at points $B$ and $C$ intersect at $P$. Lines $BP$ and $AC$ intersect at $E$. Lines $CP$ and $AB$ intersect at $F$. Let $D$ be the reflection of point $A$ in line $EF$. The circumcircles of triangles $DEB$ and $DFC$ intersect again at $Q\neq D$. Lines $DP$ and $BC$ intersect at $Z$.

Prove that points $A,Z$ and $Q$ are collinear.""",

    ("G7", "Islamic Republic of Iran"): r"""Let $ABCD$ be a cyclic quadrilateral with circumcircle $\Omega$ such that $CD>BD>BC$. The tangents to $\Omega$ at $B$ and $C$ meet at a point $S$. A line $\ell$ is called coastal if:
\begin{itemize}
...
Prove that there is a circle that is tangent to $\Omega$ and all coastal lines.""",
    ...
    ("N1", "Mongolia"): r"""Let $n$ and $k$ be positive integers such that $n<k<2n$. Suppose $a_1,a_2,\ldots,a_k$ are positive integers such that
\[
...
Here $\gcd(x,y)$ denotes the greatest common divisor of integers $x$ and $y$.""",
    ...
    ("N3", "Lithuania"): r"""An infinite sequence $a_1,a_2,\ldots$ consists of positive integers, each of which has at least four positive divisors. For each $n\geq 1$, $a_{n+1}$ is the sum of the three largest proper divisors of $a_n$. Determine all possible values of $a_1$.

A proper divisor of a positive integer $N$ is a divisor of $N$ other than $N$.""",

    ("N4", "Croatia"): r"""Let $\mathbb{Z}_{>0}$ be the set of positive integers. Determine all functions $f:\mathbb{Z}_{>0}\to\mathbb{Z}$ such that for every $n\in\mathbb{Z}_{>0}$, and every positive divisor $d$ of $n$, there exists a positive divisor $e$ of $n$ such that $n$ divides $d+f(e)$.""",

    ("N5", "India"): r"""The sequence of triples of positive integers
\[
...
Here $\gcd(x,y)$ denotes the greatest common divisor of integers $x$ and $y$.""",
    ...
    ("N6", "U.S.A."): r"""For positive integers $n$ and $k$, let $f_k(n)$ denote the remainder when $n$ is divided by $2^k-1$. A positive integer $n$ is grouse if
\[
...
\end{enumerate}""",
    ...
    ("N7", "Colombia"): r"""Let $\mathbb{Z}_{>0}$ denote the set of positive integers. A function $f:\mathbb{Z}_{>0}\to\mathbb{Z}_{>0}$ is said to be bonza if
\[
...
Determine the smallest constant $c$ such that $f(n)\leq cn$ for any bonza function $f$ and any positive integer $n$.""",
    ...
    ("N8", "Romania"): r"""Prove that there are finitely many prime numbers $p$ such that
\begin{itemize}
...
out = Path("ISL_problems/2025")
out.mkdir(parents=True, exist_ok=True)
md = ["# IMO Shortlist 2025 — problems (clean LaTeX, official booklet)\n"]
order = {"A": 0, "C": 1, "G": 2, "N": 3}
for (code, country), body in sorted(
    P.items(), key=lambda kv: (order[kv[0][0][0]], int(kv[0][0][1:]))
):
    (out / f"{code}.tex").write_text(
        f"% country: {country}\n{body.strip()}\n"
    )
